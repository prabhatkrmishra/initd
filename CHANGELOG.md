...
### Added
- `initd --status` reports every route to the daemon and whether each one is live: the unix sockets it is listening on, and each D-Bus name it holds as `listening` / `owned` / `retrying` with the reason. `initd --status --json` is the machine-readable form. This exists because there was no supported way to ask. A client that cannot find `org.freedesktop.systemd1` does not learn that the manager is unreachable - it falls through to D-Bus service activation and blocks for `service_start_timeout` (120s), which is how a third-party service manager ended up reporting "the systemd manager inspection deadline expired" while `systemctl --user` over the socket answered in 20ms. Being able to read "user dbus: retrying" turns a two-minute stall into an answer.
