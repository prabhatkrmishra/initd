...
- `install.sh` cannot hide a running daemon behind its own restart. It cleared the control sockets after a fixed 5s wait, and the check that gated the wait was `pgrep -x initd` over *every* process of the user rather than the ones it had signalled - so a daemon that appeared in that window (or a stale pid the pgrep matched after the real one exited) either got its sockets deleted while it was still serving them, or was declared gone while it was not. It also removed `initd.lock`, which is not stale state at all: the file is flocked and the kernel drops the lock when the holder dies, so deleting it only destroys mutual exclusion for the window before the new daemon starts - the exact window a second daemon would appear in. The script now records the pids it signalled, waits for those specific pids (escalating to `SIGKILL` and then waiting again, since `SIGKILL` is not instant), and refuses to clear sockets or start a second daemon if any of them is still alive. A socket that something is still serving is reported and left in place instead of removed, and the lock file is kept.
